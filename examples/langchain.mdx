...
We'll also create a `.env` file for our Dendrite API key, which is used by default by the `Dendrite` client.
...
For these, we're going to create a class called `InstagramTools`. The class will manage the Dendrite client and will contain the methods used for the tools metioned above. Let's start with creating the class and initiating Dendrite.
...
In the code above, we initiate the Dendrite client with an authenticated session on instagram.com. Read more about authentication [here](/concepts/authentication).
