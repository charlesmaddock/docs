...
To use the asynchronous Dendrite client, just import `AsyncDendrite` from `dendrite.async_api` and use it just like you would use the `Dendrite` class. It has all the same methods and all classes that stem from it will also be Async, like `AsyncPage`.
